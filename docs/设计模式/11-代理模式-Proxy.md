...
`_lastCastSecond = 0` 之后没有任何代码让它随时间增加——**技能永久冷却**，验收测试因只做连续施放而抓不到。真实游戏两种解法：① 注入时间源 `Func<int> now`，判断 `now() - _lastCastSecond < 3`；② 冷却倒计时模型（`_cooldownRemaining` 由游戏循环逐帧递减）。教训：**代理字段藏状态（冷却）时，时间源必须显式注入**。

---

`[进度：设计模式-①策略 ✓ / ②观察者 ✓ / ③装饰器 ✓ / ④工厂 ✓ / ⑤单例 ✓ / ⑥命令 ✓ / ⑦状态 ✓ / ⑧适配器+外观 ✓ / ⑨模板方法 ✓ / ⑩迭代器+组合 ✓ 核心完成 / ⑪代理模式 ✓（作业验收通过 2026-08-02）]`
